...
public class InfernalMobsCore implements ModInitializer
{
    ...
    public void onInitialize() {
        ...
        config = new Configuration(new File(FishModLoader.CONFIG_DIR, "InfernalMobs.cfg"));
        loadMods();

        new InfernalMobsServer();
        ...
        EntityEventHandler.instance = new EntityEventHandler();

        ModResourceManager.addResourcePackDomain("infernalmobs");

        Handlers.WorldLoad.register(new IWorldLoadListener() {
            @Override
            public void onWorldLoad(WorldClient world) {
                InfernalMobsClient.getInstance().onTick();
                InfernalMobsCore.instance().onTick(world);
            }
        });
        Handlers.Initialization.register(new IInitializationListener() {
            ...
            public void onClientStarted(Minecraft client) {
                loadConfig();
                proxy = new InfernalMobsClient();
                proxy.preInit();
                proxy.load();
            }
        });
        Handlers.Command.register(evt -> {
...
//        mobMods.add(MM_Ninja.class);
        ...
        config.load();
        mobMods.removeIf(c -> !config.get(Configuration.CATEGORY_GENERAL, c.getSimpleName() + " enabled", true).getBoolean(true));
        config.save();
    }
    ...
    private void parseIDsForList(String dimensionIDs, ArrayList<Integer> list)
    {
        ...
        for (String s : dimensionIDs.split(","))
        {
            ...
            {
            	FMLLog.log("InfernalMobs", Level.SEVERE, String.format("Configured DimensionID %s is not an integer! All values must be numeric. Ignoring entry", trimmedDimIDString));
            	continue;
            }
        }
    }
    ...
    public static ItemStack getItemByUnlocalizedName(String s) {
        for (Item item : Item.itemsList) {
            if (item != null) {
                if (s.equals(item.getUnlocalizedName())) {
    ...
    {
        Item item = Objects.requireNonNull(getItemByUnlocalizedName(s)).getItem();
        if (item != null)
        {
            return item;
        }
        ...
        if (!entity.worldObj.isRemote)
        {
            if (!getIsRareEntity(entity))
            {
                if (isClassAllowed(entity) && (instance.checkEntityClassForced(entity) || entity.worldObj.rand.nextInt(eliteRarity) == 0))
                {
                	...
                	{
                		FMLLog.log("InfernalMobs", Level.SEVERE, "processEntitySpawn() threw an exception");
...
//    		}
    ...
    public void sendVelocityPacket(ServerPlayer target, float xVel, float yVel, float zVel)
    {
        ...
        {
            target.playerNetServerHandler.sendPacketToPlayer(new VelocityPacket(xVel, yVel, zVel).toVanilla());
        }
    }
    ...
    public void sendKnockBackPacket(ServerPlayer target, float xVel, float zVel)
    {
        if (getIsEntityAllowedTarget(target))
        {
            target.playerNetServerHandler.sendPacketToPlayer(new KnockBackPacket(xVel, zVel).toVanilla());
        }
    }

    public void sendHealthPacket(EntityLivingBase mob, float health)
    {
        NetworkHelper.sendToAllPlayers(new HealthPacket("", mob.entityId, mob.getHealth(), mob.getMaxHealth()));
    }

    public void sendHealthRequestPacket(EntityLivingBase mob)
    {
        NetworkHelper.sendToServer(new HealthPacket(Minecraft.getMinecraft().thePlayer.getEntityName(), mob
                .entityId, 0f, 0f));
    }
    ...
    public void sendAirPacket(ServerPlayer target, int lastAir)
    {
        if (getIsEntityAllowedTarget(target))
        {
            target.playerNetServerHandler.sendPacketToPlayer(new AirPacket(lastAir).toVanilla());
        }
    }
