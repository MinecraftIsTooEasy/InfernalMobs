...
public class MM_Choke extends MobModifier
{
    ...
    public boolean onUpdate(EntityLivingBase mob)
    {
        ...
        if (lastTarget != null)
        {
            if (LegacyHelper.helperInstance.canEntityBeSeen(mob, lastTarget))
            {
                if (lastAir == -999)
                {
                ...
                if (!(lastTarget instanceof EntityPlayer && ((EntityPlayer) lastTarget).capabilities.disableDamage))
                {
                    ...
                    {
                        lastAir = 0;
                        lastTarget.attackEntityFrom(new Damage(DamageSource.drown, 2.0F));
                    }
    ...
    {
        if (lastTarget != null && source.getResponsibleEntity() == lastTarget && lastAir != -999)
        {
            lastAir += 60;
            updateAir();
        }
