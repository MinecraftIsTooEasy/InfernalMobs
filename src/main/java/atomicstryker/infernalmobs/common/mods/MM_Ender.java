...
public class MM_Ender extends MobModifier
{
    ...
    public float onHurt(EntityLivingBase mob, DamageSource source, float damage)
    {
        ...
        if (time > nextAbilityUse && source.getResponsibleEntity() != null && source.getResponsibleEntity() != mob && teleportToEntity(mob, source.getResponsibleEntity()) && !InfernalMobsCore.instance().isInfiniteLoop(mob, source.getResponsibleEntity()))
        {
            nextAbilityUse = time + coolDown;
            source.getResponsibleEntity().attackEntityFrom(new Damage(DamageSource.causeMobDamage(mob), InfernalMobsCore.instance().getLimitedDamage(damage)));
    ...
    private boolean teleportTo(EntityLivingBase mob, double destX, double destY, double destZ)
    {
        ...
        if (mob.worldObj.blockExists(x, y, z))
        {
            ...
            while (!hitGround && y < 96 && y > 0)
            {
                ...
                if (blockID != null) {
                    if (blockID.blockMaterial.isSolid()) {
                        hitGround = true;
                    } else {
                        --mob.posY;
                        --y;
        ...
        else
        {
            ...
            for (int i = 0; i < range; ++i)
            {
                ...
                mob.worldObj.spawnParticle(EnumParticle.portal_underworld, var24, var26, var28, (double) var21, (double) var22, (double) var23);
            }
