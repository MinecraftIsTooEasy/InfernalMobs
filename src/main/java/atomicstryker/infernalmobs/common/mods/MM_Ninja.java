...
public class MM_Ninja extends MobModifier
{
    ...
    public float onHurt(EntityLivingBase mob, DamageSource source, float damage)
    {
        ...
        && source.getResponsibleEntity() != null
        && source.getResponsibleEntity() != mob
        && !InfernalMobsCore.instance().isInfiniteLoop(mob, source.getResponsibleEntity())
        && teleportToEntity(mob, source.getResponsibleEntity()))
        {
    ...
    private boolean teleportTo(EntityLivingBase mob, double destX, double destY, double destZ)
    {
        ...
        if (mob.worldObj.blockExists(x, y, z))
        {
            ...
            if (hitGround)
            {
                ...
                mob.worldObj.spawnParticle(EnumParticle.hugeexplosion, oldX, oldY, oldZ, 0D, 0D, 0D);
            }
            else
            {
                return false;
